...
#if defined(NDEBUG)
# error "Liracoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xad7d5abddfb45cf735e2b8d8c147f592de42b21607d7e9fe7b0a4be8dc838f19");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Liracoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Liracoin Signed Message:\n";
...
{
    // Liracoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Liracoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Liracoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Liracoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Liracoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Liracoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7847b48545d74badd18cf0406223c92e5ac3cce3ff670809a069220673d95d7f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "28 Febbraio 2019 - Nasce la Liracoin!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041b53d455e89aba19481023233168e84cb21fbf93f4578af09d2238867cec7bc1aaf5849f7608d60bf10731fddd9ea8e1757a8d57f3712c138b6636e3b5e93e88") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1551387454;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084834083;

        if (fTestNet)
        {
            block.nTime    = 1551387378;
            block.nNonce   = 386542912;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x96ba4a38ae916d15e2d7ae60168ea811069699c8007f5051baf8a77f5c6099f2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Liracoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiracoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiracoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiracoinMiner(CWallet *pwallet)
{
    printf("LiracoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("liracoin-miner");
    ...
    try { loop {
        ...
        printf("Running LiracoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LiracoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiracoinMiner, pwallet));
}
